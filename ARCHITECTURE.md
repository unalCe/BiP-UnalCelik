...
| `image.load`, tagged `source=memory\|inflight\|network` | `ImageLoader.image(for:)` | the tallies are the cache hit rate; `inflight` joined a load already running, usually a prefetch that hadn't finished |
| `image.prefetch` | `ImagePrefetcher`, per request | outcomes matter more than durations: many `.cancelled` means over-fetching (the download still completes) |
