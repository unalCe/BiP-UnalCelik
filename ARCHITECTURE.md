...
| Nothing computes pixel sizes by hand | `ImageRequest.init(url:pointSize:scale:)` owns the arithmetic and the scale fallback; `ProductListMetrics` owns the cell geometry both renderers, the layout and the prefetcher read. A prefetch and the cell it warms cannot compute different keys by construction — previously only a test held that in line |
...
| The detail layout is written per controller, not shared | MVVM and VIPER build the same hierarchy in their own files. The duplication is deliberate: a shared `ProductDetailContentView` would put a product-shaped view in `CommonUI`, and each stack is meant to be readable end to end on its own. The *numbers* are shared — `ProductDetailMetrics` — so the three layouts cannot drift apart |
...
| 403/404 mapping depends on the endpoint | `DomainErrorMapper.map(_:for:)` takes the `ProductEndpoint`, which is already the request context. Detail keeps 403/404 → `.notFound` because S3 denies listing, so an unknown id comes back 403. The list names no product, so the same code there is an access failure and maps to `.unknown` rather than "Product not found" |
...
| Feature numbers live in the feature's Interface target | `ProductListMetrics` and `ProductDetailMetrics`. Every renderer of the feature already depends on that target, so sharing the numbers adds no edge and no module. They import `CoreGraphics` only. `ProductListLayout` (compositional layout, fonts) and `ProductGridMetrics` (`GridItem`s, skeleton counts) stay renderer-specific and read from them. **Only the numbers are shared, not the view.** The detail layout is still written out in each controller |
| `itemWidth(in:)` moved to `ProductListMetrics` | it is arithmetic over the shared numbers, and the prefetcher, the skeleton and both grids have to agree on it. Each renderer is tested against that one function: UIKit through real cell frames (`ProductListViewControllerTests`), SwiftUI by measuring a hosted `ProductGrid` (`ProductGridMetricsTests`). Before, two copies of the formula were compared with each other, which never checked what either renderer actually drew |
| `ProductGrid` wraps the SwiftUI `LazyVGrid` + padding | the content and its skeleton were building the same grid twice. Now there is one grid, and the test measures the same view the screen renders |
| The SwiftUI detail title uses `@ScaledMetric(relativeTo: .title2)` | UIKit scales `ProductDetailMetrics.titleFontSize` with `UIFontMetrics(forTextStyle: .title2)`. SwiftUI used `.title2`, which only matches while the system's title2 size is 22. Now both scale the same shared number the same way |
| Renderer-only values are named where they are used | UIKit estimated cell height stays on `ProductListLayout`. SwiftUI skeleton count and text-width fractions sit on `ProductGridMetrics`, and the UIKit skeleton's fractions on `ProductListSkeletonView`. Their values match, but each belongs to one renderer's placeholder drawing. The list image's `aspectRatio(1)` stays a literal because square is baked into `cellHeight` and the placeholder rects. A constant would suggest it can be tuned when it cannot |
...
| Unknown id returns **HTTP 403**, not 404 | S3 denies listing. On **detail**, map 403 and 404 to `.notFound`. On the list they name no product, so they are an access failure → `.unknown`. |
