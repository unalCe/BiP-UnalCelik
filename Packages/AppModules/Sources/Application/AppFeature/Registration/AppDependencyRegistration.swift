...
// container and tracer registered above them. Closures rather than
// `DependencyRegistration` types only because the container takes arguments —
// the list is still the order.
public enum AppDependencyRegistration {
    public static func register(to engine: DependencyEngine, inMemory: Bool = false) {
        let registrations: [(DependencyEngine) -> Void] = [
            PerformanceKitDependencyRegistration.register,
